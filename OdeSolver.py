...
class StochOdeSolver(object):
	...
	[more detail]
	"""
	def __init__(self, rates_update, a, x0, knockdown, steps, mu):
		"""Builder"""
		self.params = a
		self.a = copy.deepcopy(self.params)
		self.rates_update = rates_update
		self.x0 = x0
		if np.shape(self.x0) == ():
			self.x0 = np.array([self.x0])
		self.rates = self.rates_update(self.x0, self.a)
	...
	def gillespie_time(self, max_time, start_time=0, warnings=False):
		...
		time = start_time
		test = []
		x_temp = copy.copy(self.x[i-1])
		while time < max_time:
			if(self.t[-1] >= self.knockdown['time_on'] and self.kd_off):
				self.a[-1] = self.knockdown
				self.kd_off = False
